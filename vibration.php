...
$sensorDef = [
    ...
    2 => ['axis' => 'Y', 'color' => '#1cc88a', 'icon' => 'fas fa-arrows-alt-v', 'desc' => 'Vertical'],
    3 => ['axis' => 'Z', 'color' => '#f6c23e', 'icon' => 'fas fa-expand-arrows-alt', 'desc' => 'Axial'],
    4 => ['axis' => 'B', 'color' => '#e74a3b', 'icon' => 'fas fa-dot-circle', 'desc' => 'Bearing'],
];
...
if ($machineId) {
    ...
    foreach ($lq->fetchAll() as $r)
        $latestSensors[(int) $r['sensor_num']] = $r;
}
...
<div class="d-flex flex-wrap align-items-start justify-content-between mb-3" style="gap:10px;">
    <div>
        <h1 class="h3 mb-0 text-gray-800">
            <i class="fas fa-wave-square mr-2"></i>Vibration Monitoring
        </h1>
    ...
    <div class="d-flex flex-wrap align-items-center" style="gap:6px;">
        ...
        <small id="liveLabel" class="text-muted">Inactive</small>
...
<?php if ($anyAlert): ?>
    <div class="alert alert-danger alert-dismissible fade show mb-3" style="border-left:4px solid #e74a3b;">
        ...
        <strong>Warning!</strong> Vibration detected above safe ISO 10816 limits.
...
<div class="d-flex flex-wrap align-items-center mb-3" style="gap:8px;font-size:.8rem;">
    ...
    <span class="text-muted ml-1">RMS per physical sensor</span>
</div>
...
<div class="row mb-4">
    <?php foreach ($sensorDef as $sn => $def):
        ...
        <div class="col-xl-3 col-md-6 mb-3">
            <div class="card shadow h-100" id="card-s<?= $sn ?>" style="border-left:4px solid <?= $def['color'] ?>;">
                ...
                <div class="card-header py-2 d-flex align-items-center justify-content-between" style="background:#f8f9fc;">
                    <div>
                        ...
                        <span class="badge badge-light ml-1"
                            style="font-size:.7rem;border:1px solid <?= $def['color'] ?>;color:<?= $def['color'] ?>;">
                            Axis <?= $def['axis'] ?>
                        </span>
                        <small class="text-muted ml-1" style="font-size:.68rem;"><?= $def['desc'] ?></small>
                    </div>
                    <span id="badge-s<?= $sn ?>"
                        class="badge badge-<?= $st === 'critical' ? 'danger' : ($st === 'warning' ? 'warning' : 'success') ?>">
                        <?= $hasData ? ucfirst($st) : '—' ?>
                    </span>
                </div>
                ...
                <div class="card-body text-center py-3 px-3">
                    <!-- RMS value -->
                    <div id="rms-s<?= $sn ?>" class="sensor-rms mb-0" style="color:<?= $hasData ? $clr : '#ccc' ?>;">
                    ...
                    <div class="mt-3 mb-1">
                        <div class="iso-track">
                            ...
                            <div style="position:absolute;left:28%;top:0;bottom:0;width:2px;background:rgba(0,0,0,.2);">
                            </div>
                            <div style="position:absolute;left:71%;top:0;bottom:0;width:2px;background:rgba(0,0,0,.2);">
                            </div>
                        </div>
                    ...
                    <?php if ($hasData): ?>
                        <div class="row no-gutters text-center mt-2 pt-2"
                            style="font-size:.75rem;border-top:1px solid #f0f0f0;">
                            <?php
                            $subAxes = [
                                'X' => $r['sensor_1'],
                                'Y' => $r['sensor_2'],
                    ...
                    <?php else: ?>
                        <div class="text-muted mt-3" style="font-size:.78rem;">
                            <i class="fas fa-satellite-dish mr-1"></i>Waiting for data from ESP32<br>
                    ...
                    <small id="ts-s<?= $sn ?>" class="text-muted" style="font-size:.7rem;">
                        <?= $hasData ? date('d/m H:i:s', strtotime($r['recorded_at'])) : 'No data yet' ?>
                    </small>
                ...
                <div class="card-footer py-1 px-3 d-flex justify-content-between"
                    style="font-size:.7rem;background:#f8f9fc;">
                    ...
                    <span>
                        <?php if ($ss['crit'] > 0): ?><span class="badge badge-danger"
                                style="font-size:.6rem;"><?= $ss['crit'] ?>×Crit</span><?php endif; ?>
                        <?php if ($ss['warn'] > 0): ?><span class="badge badge-warning"
                                style="font-size:.6rem;"><?= $ss['warn'] ?>×Warn</span><?php endif; ?>
                        <?php if ($ss['crit'] == 0 && $ss['warn'] == 0 && $ss['avg'] > 0): ?><span class="badge badge-success"
                                style="font-size:.6rem;">OK</span><?php endif; ?>
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="card shadow">
            <div class="card-header py-2 d-flex align-items-center justify-content-between">
                <h6 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-chart-line mr-1"></i>24-Hour RMS Trend
                </h6>
                <div class="d-flex align-items-center" style="gap:6px;">
                    <?php foreach ($sensorDef as $sn => $def): ?>
                        <button class="btn btn-sm axis-toggle active" data-sn="<?= $sn ?>" style="padding:2px 8px;font-size:.72rem;
    ...
    <div class="col-lg-4 mb-3 d-flex flex-column" style="gap:12px;">
        ...
        <div class="card shadow">
            <div class="card-header py-2">
                <h6 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-record-vinyl mr-1"></i>Measurement Session
                </h6>
            </div>
            <div class="card-body py-3">
                <div class="form-row mb-2">
                    ...
                    <div class="col-7">
                        <label class="small font-weight-bold mb-1">Measurement Point Label</label>
                ...
                <small class="text-muted d-block" id="sessStatus">No active session</small>
        ...
        <div class="card shadow flex-fill">
            <div class="card-header py-2">
                <h6 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-clipboard-check mr-1"></i>24-Hour Summary
                </h6>
            </div>
...
<div class="card shadow mb-4">
    <div class="card-header py-3 d-flex justify-content-between align-items-center">
        <h6 class="m-0 font-weight-bold text-primary">
            <i class="fas fa-table mr-1"></i>Data History
            <small class="text-muted font-weight-normal ml-2">
                <?= htmlspecialchars($dateFrom) ?> to <?= htmlspecialchars($dateTo) ?>
            </small>
        </h6>
        <small class="text-muted"><?= count($tableData) ?> rows</small>
    </div>
    <div class="card-body p-0">
            ...
            <table class="table table-bordered table-sm mb-0" id="vibTable" width="100%">
                ...
                <tbody>
                    <?php if (empty($tableData)): ?>
                        <tr>
                            <td colspan="11" class="text-center text-muted py-3">No data</td>
                        </tr>
                    <?php else:
                        foreach ($tableData as $row):
                            ...
                            <tr class="<?= $rc ?>">
                                <td style="white-space:nowrap;font-size:.78rem;">
                                    <?= date('d/m/Y H:i:s', strtotime($row['recorded_at'])) ?></td>
                                <td><strong style="color:<?= $axColor ?>;">S<?= $sn2 ?></strong></td>
                                <td><span class="badge badge-light"
                                        style="border:1px solid <?= $axColor ?>;color:<?= $axColor ?>;font-size:.7rem;">Axis
                                        <?= $axLabel ?></span></td>
                                <td><strong><?= $row['rms_overall'] !== null ? number_format((float) $row['rms_overall'], 3) : '—' ?></strong>
                                </td>
                                ...
                                <td><?= $row['temp_sensor'] !== null ? number_format((float) $row['temp_sensor'], 1) . '°C' : '—' ?>
                                </td>
                            </tr>
...
<script>
    ...
    $(document).ready(function () {
        ...
        if (ctx && typeof Chart !== 'undefined') {
            ...
            datasets.push({
                label: 'Warning (2.8)', data: Array(n).fill(VIB_WARN_T),
                borderColor: '#f6c23e', borderDash: [6, 4], borderWidth: 1, pointRadius: 0, fill: false, spanGaps: true
            });
            datasets.push({
                label: 'Critical (7.1)', data: Array(n).fill(VIB_CRIT_T),
    ...
    function toggleLive() {
        ...
        if (liveActive) {
            ...
            lbl.textContent = 'Live · 500ms';
            liveTimer = setInterval(doLive, 500);
            doLive();
        } else {
            ...
            .then(function (d) {
                ...
                d.data.forEach(function (row) {
                    ...
                    if (!_lastIds[sn] || _lastIds[sn] != row.id) {
                        ...
                        _lastIds[sn] = row.id;
                        if (_sessId) { _sessCount++; setTxt('sessCount', _sessCount + ' readings'); }
                    }
                });
            })
            .catch(function () { })
    ...
    function startSession() {
        if (!VIB_MID) { alert('Please select a machine.'); return; }
        ...
        fetch('api/vibration_portable.php?action=session_start&machine_id=' + VIB_MID +
            ...
            .then(function (d) {
                if (d.success) {
                    ...
                    setTxt('sessCount', '0 readings');
                }
            });
    }
    function endSession() {
        if (!_sessId) { alert('No active session.'); return; }
        fetch('api/vibration_portable.php?action=session_end&session_id=' + _sessId)
            .then(function (r) { return r.json(); })
            .then(function (d) {
                if (d.success) {
                    var s = d.summary || {};
                    setTxt('sessStatus', 'Session #' + _sessId + ' completed');
