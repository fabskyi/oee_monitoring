...
foreach ($machines as $m) {
    $ln = $m['line_name'] ?? 'Umum';
    if (!isset($lineStats[$ln])) $lineStats[$ln] = ['total'=>0,'run'=>0,'oee_sum'=>0,'oee_cnt'=>0];
    $lineStats[$ln]['total']++;
    if ($m['status'] === 'run') $lineStats[$ln]['run']++;
    if ($m['oee_pct'] > 0) { $lineStats[$ln]['oee_sum'] += $m['oee_pct']; $lineStats[$ln]['oee_cnt']++; }
}
foreach ($lineStats as &$ls) $ls['avg_oee'] = $ls['oee_cnt'] ? round($ls['oee_sum']/$ls['oee_cnt'],1) : 0;
unset($ls);
...
// Mesin dengan kondisi tidak normal (vibration warning/critical atau status stop)
...
// Bangun lookup [machine_id][snap_date] = oee
$trendMap = [];
foreach ($trendRaw as $r) {
    $trendMap[$r['machine_id']][$r['snap_date']] = (float)$r['oee'];
}

// Build chart dataset per mesin — hanya mesin yg punya data trend
$chartMachines = [];
foreach ($machines as $m) {
    if (!isset($trendMap[$m['id']])) continue;
    $pts = [];
    foreach ($dateRows as $d) $pts[] = $trendMap[$m['id']][$d] ?? null;
    // Lewati jika semua null
    if (count(array_filter($pts, fn($v) => $v !== null)) === 0) continue;
    $chartMachines[] = [
        'id'   => $m['id'],
        'name' => $m['name'],
        'line' => $m['line_name'] ?? 'Umum',
        'data' => $pts,
...
:root {
  ...
  /* Semantic */
  --ok:         #1a7f54;   /* green  – running / normal */
  --ok-bg:      #e6f7ef;
  --ok-border:  #a8dfc3;
  --warn:       #9a6400;   /* amber  – warning */
  --warn-bg:    #fef8e7;
  --warn-border:#f5d87a;
  --danger:     #b91c1c;   /* red    – critical / stop */
  --danger-bg:  #fef2f2;
...
html, body { width:100%; height:100%; overflow:hidden; background:var(--bg); color:var(--text-2); font-family:var(--font); }
...
/* Ticker row di bawah dalam header */
.hdr-ticker {
  display: flex;
  align-items: center;
  gap: 10px;
...
.hdr-ticker .footer-lbl {
  font-size: .52rem; text-transform: uppercase; letter-spacing: 2px;
  color: rgba(255,255,255,.7); white-space: nowrap; font-weight: 800; flex-shrink: 0;
}
.hdr-ticker .ticker-wrap { flex: 1; overflow: hidden; white-space: nowrap; }
.hdr-ticker .ticker-inner {
  display: inline-block;
  animation: ticker 90s linear infinite;
  font-size: .6rem; color: rgba(255,255,255,.6);
}
.hdr-ticker .ticker-inner strong { color: rgba(255,255,255,.9); }
...
.hdr-brand-icon img { width:100%; height:100%; object-fit:contain; }
.hdr-brand-text h1 { font-size:1.15rem; font-weight:800; letter-spacing:.8px; color:#fff; }
.hdr-brand-text p  { font-size:.65rem; color:rgba(255,255,255,.65); letter-spacing:1.5px; text-transform:uppercase; margin-top:2px; }

/* Divider */
.hdr-div { width:1px; height:36px; background:rgba(255,255,255,.2); margin:0 18px; flex-shrink:0; }

/* Stats cluster */
.hdr-stats { display:flex; align-items:center; gap:0; flex:1; }
.hstat { text-align:center; padding:0 14px; }
.hstat .v { font-size:1.85rem; font-weight:900; line-height:1; color:#fff; }
.hstat .v.ok   { color:#fff; }
.hstat .v.danger { color:#fff; }
.hstat .v.warn { color:#fff; }
.hstat .v.gold { color:#fff; }
.hstat .l { font-size:.62rem; text-transform:uppercase; letter-spacing:1px; color:rgba(255,255,255,.6); margin-top:3px; }

/* Clock */
.hdr-clock { flex-shrink:0; text-align:right; }
#tvClock { font-size:2.1rem; font-weight:900; color:#fff; letter-spacing:3px; font-variant-numeric:tabular-nums; line-height:1; }
#tvDate  { font-size:.72rem; color:rgba(255,255,255,.75); margin-top:3px; }
#tvSync  { font-size:.5rem;  color:rgba(255,255,255,.4); }
...
.badge-live {
  ...
  font-size: .52rem; padding: 2px 8px; border-radius:20px;
...
.scroll-viewport:hover .scroll-track { animation-play-state: paused !important; }
...
.mc.run::before  { opacity: .5; }
.mc.stop::before { opacity: .18; }

/* Left accent — single neutral */
.mc.oee-ex, .mc.oee-gd, .mc.oee-ok, .mc.oee-bd {
  border-left: 3px solid var(--border-dk);
}

/* Line + Name */
.mc-line {
  font-size: .62rem; font-weight: 700; color: var(--text-1);
...
.mc-oee-unit { font-size: 1rem; font-weight: 700; color: var(--text-3); margin-bottom: 4px; }
.mc-oee-lbl  { font-size: .58rem; text-transform: uppercase; letter-spacing: 1.5px; color: var(--text-1); font-weight:700; margin-top: 3px; text-align:center; }

/* ── A / P / Q bars ── */
.mc-bars { margin-bottom: 10px; display:flex; flex-direction:column; gap:5px; }
.bar-row { display:flex; align-items:center; gap:7px; }
.bar-lbl { font-size: .65rem; font-weight: 700; color: var(--text-3); width:16px; text-align:right; flex-shrink:0; }
.bar-track { flex:1; height:7px; background:#eef0f3; border-radius:4px; overflow:hidden; }
.bar-fill  { height:100%; border-radius:4px; transition: width 1s ease; }
.bar-val   { font-size: .65rem; font-weight: 800; width:34px; text-align:right; flex-shrink:0; }
...
.snsr-lbl { font-size: .58rem; color: var(--text-4); font-weight: 600; text-transform:uppercase; letter-spacing:.5px; }
.snsr-val { font-size: .85rem; font-weight: 800; color: var(--text-1); line-height: 1.2; margin-top: 1px; }
...
.status-pill.run  { background:#f0f2f5; color:#374151; border: 1px solid var(--border-dk); }
.status-pill.stop { background:var(--maroon-lt); color:var(--maroon); border: 1px solid #ddbdbd; }
.status-pill i { font-size: .55rem; }

.vib-pill {
  font-size: .6rem; font-weight: 700; padding: 2px 8px; border-radius:12px;
}
.vib-pill.normal   { background:#f0f2f5; color:var(--text-3); border:1px solid var(--border); }
.vib-pill.warning  { background:var(--maroon-lt); color:var(--maroon); border:1px solid #ddbdbd; }
.vib-pill.critical { background:var(--maroon); color:#fff; animation:blink .8s infinite; }

.mc-age { font-size:.58rem; color:var(--text-4); white-space:nowrap; }

/* ── Scroll control buttons ── */
.scroll-ctrl { display:flex; align-items:center; gap:4px; }
.scroll-ctrl button {
  display:inline-flex; align-items:center; justify-content:center;
  width:24px; height:24px; border-radius:6px; border:1px solid var(--border-dk);
  background:var(--surface); color:var(--text-2); cursor:pointer;
  font-size:.7rem; line-height:1; transition:background .15s,color .15s;
  padding:0;
}
.scroll-ctrl button:hover { background:var(--maroon); color:#fff; border-color:var(--maroon); }
.scroll-ctrl button.pause-active { background:var(--maroon-lt); color:var(--maroon); border-color:#ddbdbd; }
...
.side-hdr .ttl { font-size:.68rem; font-weight:700; text-transform:uppercase; letter-spacing:1.5px; color:var(--text-3); }
.side-body { flex:1; min-height:0; overflow:hidden; }
...
.sum-box .sv { font-size: 1.4rem; font-weight: 900; line-height:1; }
.sum-box .sl { font-size: .55rem; color:var(--text-4); text-transform:uppercase; letter-spacing:.8px; margin-top:2px; }
...
.dist-legend {
  ...
  padding: 5px 12px 8px; font-size:.58rem; color:var(--text-3);
}
.dist-legend span { display:inline-flex; align-items:center; gap:4px; }
.dot { width:7px; height:7px; border-radius:50%; flex-shrink:0; }

/* Line breakdown table */
.line-divider { height:1px; background:var(--border); margin:0 12px; }
.line-title { font-size:.6rem; font-weight:700; text-transform:uppercase; letter-spacing:1.5px; color:var(--text-4); padding:8px 12px 5px; }
.line-table { width:100%; border-collapse:collapse; }
.line-table td { padding:5px 12px; font-size:.7rem; border-bottom:1px solid var(--border); }
.line-table tr:last-child td { border-bottom:none; }
.line-name-cell { font-weight:700; color:var(--text-2); }
.line-run-cell  { text-align:center; }
.line-oee-cell  { text-align:right; font-weight:800; }
.line-bar-cell  { width:70px; }
.line-mini-bar  { height:5px; background:#eef0f3; border-radius:3px; overflow:hidden; }
.line-mini-fill { height:100%; border-radius:3px; }

/* ── Chart ── */
.chart-card { flex: 0 0 185px; }
.chart-wrap { height:100%; padding:8px 10px 6px; position:relative; }

/* ── Alert Panel ── */
.alert-card { flex:1; min-height:0; }
.alert-list { height:100%; overflow:hidden; display:flex; flex-direction:column; }
.alert-row {
...
.alert-row:first-child { background:var(--surface-2); }
.a-sev {
  flex-shrink:0; width:52px; padding:2px 0; border-radius:4px;
  text-align:center; font-size:.58rem; font-weight:800; text-transform:uppercase;
}
.a-sev.critical { background:var(--danger); color:#fff; }
.a-sev.warning  { background:#f59e0b; color:#fff; }
.a-sev.info     { background:var(--info); color:#fff; }
.a-mach { flex:1; }
.a-mach .name { font-size:.72rem; font-weight:700; color:var(--text-1); }
.a-mach .key  { font-size:.6rem; color:var(--text-4); }
.a-val  { font-size:.75rem; font-weight:800; color:var(--maroon); width:52px; text-align:right; flex-shrink:0; }
.a-time { font-size:.58rem; color:var(--text-4); width:36px; text-align:right; flex-shrink:0; }
.no-alert {
  flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:8px;
}
.no-alert i   { font-size:1.8rem; color:var(--ok); }
.no-alert .t1 { font-size:.8rem; font-weight:700; color:var(--ok); }
.no-alert .t2 { font-size:.62rem; color:var(--text-4); }

@keyframes ticker { 0%{transform:translateX(60vw)} 100%{transform:translateX(-100%)} }

/* ── AJAX smooth update transitions ── */
.mc-oee-val, .bar-fill, .bar-val, .snsr-val, .status-pill, .vib-pill {
  transition: all .6s ease;
}
.mc { transition: box-shadow .2s, transform .2s, border-left-color .6s; }
.val-flash { animation: valFlash .8s ease; }
@keyframes valFlash {
  0%   { opacity:1; }
  30%  { opacity:.25; }
  100% { opacity:1; }
...
#syncDot {
  ...
  background:#6ee7b7; margin-left:6px; vertical-align:middle;
  transition: background .3s;
}
#syncDot.syncing { background:#fde68a; animation:blink .4s infinite; }
#syncDot.error   { background:#fca5a5; }
</style>
</head>
<body>
<div class="tv">
...
<div class="hdr-top">
  ...
  <div class="hdr-stats">
    <div class="hstat"><div id="hStat-total"  class="v gold"><?= $totalMachines ?></div><div class="l">Total Mesin</div></div>
    <div class="hdr-div"></div>
    <div class="hstat"><div id="hStat-run"    class="v ok"><?= $machineRun ?></div><div class="l">Running</div></div>
    <div class="hdr-div"></div>
    <div class="hstat"><div id="hStat-stop"   class="v danger"><?= $machineStop ?></div><div class="l">Stopped</div></div>
    <div class="hdr-div"></div>
    <div class="hstat"><div id="hStat-avgOee" class="v gold"><?= number_format((float)$avgOEE,1) ?>%</div><div class="l">Avg OEE</div></div>
    <div class="hdr-div"></div>
    <div class="hstat"><div id="hStat-ex"     class="v ok"><?= $oeeExcellent ?></div><div class="l">OEE ≥85%</div></div>
    <div class="hdr-div"></div>
    <div class="hstat"><div id="hStat-good"   class="v warn"><?= $oeeGood ?></div><div class="l">OEE 65–85%</div></div>
    <div class="hdr-div"></div>
    <div class="hstat"><div id="hStat-poor"   class="v danger"><?= $oeePoor ?></div><div class="l">OEE &lt;65%</div></div>
    <div class="hdr-div"></div>
    <div class="hstat"><div id="hStat-alert"  class="v <?= $alertCritical > 0 ? 'danger' : ($alertWarning > 0 ? 'warn' : 'ok') ?>"><?= $alertCount ?></div><div class="l">Alert Aktif</div></div>
  </div>
...
<!-- Ticker berjalan di bawah header -->
  ...
  <div class="ticker-wrap">
    <span class="ticker-inner">
      <?php foreach ($machines as $m): ?>
        ...
        [<?= strtoupper($m['status']) ?>]&nbsp;&#124;
      <?php endforeach; ?>
      &nbsp;&nbsp;&#9632;&nbsp;Auto-refresh 30s&nbsp;&#9632;&nbsp;Update: <?= date('d M Y H:i:s') ?>
    </span>
  </div>
</div>
...
<div class="body">
  ...
  <div class="card-panel">
    <div class="card-panel-hdr">
      <span class="ttl">
        ...
        Status Mesin &mdash; <?= $totalMachines ?> Unit
        <span style="color:var(--text-4);font-weight:400;margin-left:6px;">· hover untuk pause scroll</span>
      </span>
      <div style="display:flex;align-items:center;gap:8px;">
        <div class="scroll-ctrl">
          <button onclick="cardScrollUp()" title="Scroll ke atas">&#8593;</button>
          <button id="cardPauseBtn" onclick="cardTogglePause()" title="Pause/Resume">&#9646;&#9646;</button>
          <button onclick="cardScrollDown()" title="Scroll ke bawah">&#8595;</button>
        </div>
    ...
    <div class="scroll-viewport" id="scrollVP">
      <div class="scroll-track" id="scrollTrack">
        ...
        <?php foreach ($machines as $m):
          ...
          // Bar colors — hijau/kuning/merah sesuai nilai
          ...
          // Temp color — hanya merah jika > 50°C (bahaya nyata)
          $tmpC = ($m['temp_panel'] ?? 0) > 50 ? 'var(--maroon)' : 'var(--text-1)';
        ?>
        <div class="mc <?= $m['status'] ?> <?= $oeeClass ?>" data-machine-id="<?= $m['id'] ?>">

          <div class="mc-line"><?= htmlspecialchars($m['line_name'] ?? 'Umum') ?></div>
          ...
          <div class="mc-bars">
            <div class="bar-row">
              ...
              <div class="bar-track"><div class="bar-fill" style="width:<?= min($avl,100) ?>%;background:<?= $bc($avl) ?>;"></div></div>
              <span class="bar-val" style="color:<?= $bc($avl) ?>;"><?= $avl > 0 ? $avl.'%' : '—' ?></span>
            </div>
            <div class="bar-row">
              <span class="bar-lbl">P</span>
              <div class="bar-track"><div class="bar-fill" style="width:<?= min($prf,100) ?>%;background:<?= $bc($prf) ?>;"></div></div>
              <span class="bar-val" style="color:<?= $bc($prf) ?>;"><?= $prf > 0 ? $prf.'%' : '—' ?></span>
            </div>
            <div class="bar-row">
              <span class="bar-lbl">Q</span>
              <div class="bar-track"><div class="bar-fill" style="width:<?= min($qlt,100) ?>%;background:<?= $bc($qlt) ?>;"></div></div>
          ...
          <div class="mc-sensors">
            ...
            <div class="snsr-box">
              ...
              <div class="snsr-val" style="color:<?= $tmpC ?>;"><?= $m['temp_panel'] !== null ? number_format($m['temp_panel'],1).' °C' : '—' ?></div>
            </div>
            <div class="snsr-box">
              <div class="snsr-lbl">Humidity</div>
              <div class="snsr-val"><?= $m['hum_panel'] !== null ? number_format($m['hum_panel'],1).' %' : '—' ?></div>
            </div>
          </div>
  ...
  <div class="right">
    ...
    <!-- Ringkasan + Line Breakdown -->
    <div class="side-card" style="flex:0 0 auto;">
      <div class="side-hdr">
        <span class="ttl"><i class="fas fa-layer-group" style="margin-right:6px;color:var(--maroon);"></i>Ringkasan & Breakdown Line</span>
      </div>
      <!-- Top 3 stat boxes -->
      <div class="summary-top">
        ...
        <div class="sum-box">
          <div id="sum-alert" class="sv" style="color:<?= $alertCount > 0 ? 'var(--danger)' : 'var(--ok)' ?>;"><?= $alertCount ?></div>
          <div class="sl">Alert Aktif</div>
        </div>
      </div>
      ...
      <div class="dist-bar" style="margin:4px 12px 0;">
        <div id="dist-ex"   class="dist-seg" style="width:<?= round($oeeExcellent/$tot*100) ?>%;background:var(--maroon);opacity:.9;"></div>
        <div id="dist-good" class="dist-seg" style="width:<?= round($oeeGood/$tot*100) ?>%;background:var(--maroon);opacity:.5;"></div>
        <div id="dist-poor" class="dist-seg" style="width:<?= round($oeePoor/$tot*100) ?>%;background:var(--maroon);opacity:.2;"></div>
        <div class="dist-seg" style="flex:1;background:#eef0f3;"></div>
      </div>
      <div class="dist-legend">
        <span><span class="dot" style="background:var(--maroon);opacity:.9;"></span><span id="dist-ex-lbl">≥85% (<?= $oeeExcellent ?>)</span></span>
        <span><span class="dot" style="background:var(--maroon);opacity:.5;"></span><span id="dist-good-lbl">65–85% (<?= $oeeGood ?>)</span></span>
        <span><span class="dot" style="background:var(--maroon);opacity:.2;border:1px solid #ccc;"></span><span id="dist-poor-lbl">&lt;65% (<?= $oeePoor ?>)</span></span>
      </div>

      <!-- Per-line breakdown -->
      <div class="line-divider"></div>
      <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 12px 4px;">
        <span class="line-title" style="padding:0;">Breakdown per Production Line</span>
        <div class="scroll-ctrl">
          <button onclick="lineScrollUp()" title="Scroll ke atas">&#8593;</button>
          <button id="linePauseBtn" onclick="lineTogglePause()" title="Pause/Resume">&#9646;&#9646;</button>
          <button onclick="lineScrollDown()" title="Scroll ke bawah">&#8595;</button>
        </div>
      </div>
      ...
      <table class="line-table">
        <thead>
          <tr style="background:var(--surface-2);">
            <td style="font-size:.6rem;font-weight:700;color:var(--text-4);text-transform:uppercase;letter-spacing:1px;">Line</td>
            <td style="font-size:.6rem;font-weight:700;color:var(--text-4);text-transform:uppercase;letter-spacing:1px;text-align:center;">Run/Total</td>
            <td style="font-size:.6rem;font-weight:700;color:var(--text-4);text-transform:uppercase;letter-spacing:1px;text-align:right;">Avg OEE</td>
        ...
        <tr data-line="<?= rawurlencode($lineName) ?>">
          ...
          <td class="line-oee-cell" style="color:<?= $oeeC2 ?>;"><?= $ls['avg_oee'] > 0 ? $ls['avg_oee'].'%' : '—' ?></td>
          <td class="line-bar-cell">
            <div class="line-mini-bar">
              <div class="line-mini-fill" style="width:<?= min($ls['avg_oee'],100) ?>%;background:<?= $fillC ?>;"></div>
            </div>
          </td>
        </tr>
        <?php endforeach; ?>
    ...
    <!-- OEE Trend Chart per mesin -->
    <div class="side-card chart-card">
      <div class="side-hdr" style="justify-content:space-between;">
        <span class="ttl"><i class="fas fa-chart-line" style="margin-right:6px;color:var(--maroon);"></i>Tren OEE 14 Hari — per Mesin</span>
        <div style="display:flex;align-items:center;gap:6px;">
          <button onclick="prevMachine()" style="background:var(--surface-2);border:1px solid var(--border);border-radius:5px;width:22px;height:22px;cursor:pointer;font-size:.65rem;line-height:1;color:var(--text-3);">&#8249;</button>
          <span id="chartMachIdx" style="font-size:.6rem;color:var(--text-4);min-width:48px;text-align:center;">1 / ?</span>
          <button onclick="nextMachine()" style="background:var(--surface-2);border:1px solid var(--border);border-radius:5px;width:22px;height:22px;cursor:pointer;font-size:.65rem;line-height:1;color:var(--text-3);">&#8250;</button>
          <span id="chartAutoIcon" style="font-size:.55rem;color:var(--text-4);margin-left:2px;" title="Auto-scroll aktif">&#9654; AUTO</span>
        </div>
      </div>
      <!-- machine info bar -->
      <div id="chartMachInfo" style="padding:5px 12px 4px;border-bottom:1px solid var(--border);background:var(--surface-2);display:flex;align-items:center;gap:8px;flex-shrink:0;">
      ...
      <div style="padding:3px 12px 0;background:var(--surface-2);flex-shrink:0;">
        <div style="height:3px;background:#eef0f3;border-radius:2px;overflow:hidden;">
          <div id="chartProgressFill" style="height:100%;width:0%;background:var(--maroon);border-radius:2px;transition:width .4s ease;"></div>
        </div>
      </div>
    ...
    <div class="side-card alert-card">
      <div class="side-hdr">
        <span class="ttl"><i class="fas fa-exclamation-triangle" style="margin-right:6px;color:var(--maroon);"></i>Kondisi Mesin Tidak Normal</span>
        <div style="display:flex;align-items:center;gap:6px;">
          <span style="font-size:.6rem;color:var(--text-4);"><?= count($abnormalMachines) ?> mesin</span>
          <div class="scroll-ctrl">
            <button onclick="alertScrollUp()" title="Scroll ke atas">&#8593;</button>
            <button id="alertPauseBtn" onclick="alertTogglePause()" title="Pause/Resume">&#9646;&#9646;</button>
            <button onclick="alertScrollDown()" title="Scroll ke bawah">&#8595;</button>
          </div>
        </div>
      </div>
      <!-- Sub-header kolom — fixed, tidak ikut scroll -->
      <div style="display:grid;grid-template-columns:1fr 80px 1fr;gap:0;padding:5px 14px 4px;background:var(--surface-2);border-bottom:2px solid var(--border-dk);flex-shrink:0;">
        <span style="font-size:.58rem;font-weight:700;color:var(--text-4);text-transform:uppercase;letter-spacing:.8px;">Mesin</span>
        <span style="font-size:.58rem;font-weight:700;color:var(--text-4);text-transform:uppercase;letter-spacing:.8px;text-align:center;">Kondisi</span>
        <span style="font-size:.58rem;font-weight:700;color:var(--text-4);text-transform:uppercase;letter-spacing:.8px;text-align:left;padding-left:12px;">Keterangan</span>
      </div>
      <div class="side-body" id="alertVP" style="overflow:hidden;position:relative;">
        ...
        <?php if (empty($abnormalMachines)): ?>
          <div class="no-alert">
            ...
            <div class="t1">Semua Mesin Normal</div>
            <div class="t2">Tidak ada kondisi abnormal terdeteksi</div>
          </div>
        <?php else: ?>
          <?php foreach ($abnormalMachines as $ab):
            ...
            if ($vs === 'critical' || $vs === 'warning') {
              ...
              $axisLabel  = $vs === 'critical' ? '⚠ KRITIS' : '⚠ TINGGI';
              if ($vx !== null) $issues[] = ['sev'=>$vs, 'txt'=> 'Vib X: '.number_format($vx,2).' mm/s'.($vx>7.1?' (kritis)':($vx>2.8?' (warning)':''))];
              if ($vy !== null) $issues[] = ['sev'=>$vs, 'txt'=> 'Vib Y: '.number_format($vy,2).' mm/s'.($vy>7.1?' (kritis)':($vy>2.8?' (warning)':''))];
              if ($vz !== null) $issues[] = ['sev'=>$vs, 'txt'=> 'Vib Z: '.number_format($vz,2).' mm/s'.($vz>7.1?' (kritis)':($vz>2.8?' (warning)':''))];
              if ($rms > 0)     $issues[] = ['sev'=>$vs, 'txt'=> 'RMS Overall: '.number_format($rms,2).' mm/s'];
            }

            // Status stop
            if ($ab['status'] === 'stop')
              $issues[] = ['sev'=>'stop', 'txt'=> 'Mesin tidak beroperasi'];

            // Tegangan abnormal (<190V atau >240V per phase)
            foreach (['R'=>$vr,'S'=>$vs_,'T'=>$vt] as $ph=>$val) {
              if ($val !== null && ($val < 190 || $val > 240))
                $issues[] = ['sev'=>'warn', 'txt'=> 'Tegangan Ph-'.$ph.': '.number_format($val,1).' V'];
            }

            // Arus tinggi (>30A)
            foreach (['R'=>$ar,'S'=>$as_,'T'=>$at] as $ph=>$val) {
              if ($val !== null && $val > 30)
                $issues[] = ['sev'=>'warn', 'txt'=> 'Arus Ph-'.$ph.': '.number_format($val,2).' A'];
            }

            // Suhu panel tinggi
            if ($tmp > 50) $issues[] = ['sev'=>'critical', 'txt'=> 'Suhu Panel: '.number_format($tmp,1).' °C'];
            elseif ($tmp > 40) $issues[] = ['sev'=>'warn', 'txt'=> 'Suhu Panel: '.number_format($tmp,1).' °C'];

            // Kelembaban tinggi
            if ($hum > 80) $issues[] = ['sev'=>'warn', 'txt'=> 'Kelembaban: '.number_format($hum,1).' %'];

            // Tentukan level & warna badge
            $hasCrit = $vs === 'critical' || $tmp > 50;
            $hasWarn = $vs === 'warning' || count($issues) > 0;
            if ($hasCrit) {
              $level = 'CRITICAL'; $lColor = '#dc2626'; $lBg = '#fef2f2'; $lBorder = '#fca5a5'; $blink = 'animation:blink .8s infinite;';
            } elseif ($ab['status'] === 'stop') {
              $level = 'STOP';     $lColor = 'var(--maroon)'; $lBg = 'var(--maroon-lt)'; $lBorder = '#ddbdbd'; $blink = '';
            } elseif ($hasWarn) {
              $level = 'WARNING';  $lColor = '#d97706'; $lBg = '#fffbeb'; $lBorder = '#fde68a'; $blink = '';
            } else {
              $level = 'INFO';     $lColor = '#374151'; $lBg = '#f0f2f5'; $lBorder = 'var(--border-dk)'; $blink = '';
            }
          ?>
          <div style="display:grid;grid-template-columns:140px 80px 1fr;align-items:start;gap:0;padding:8px 14px;border-bottom:1px solid var(--border);">
            <!-- Nama + Line -->
            <div style="padding-top:2px;">
              <div style="font-size:.8rem;font-weight:800;color:var(--text-1);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?= htmlspecialchars($ab['name']) ?></div>
              <div style="font-size:.58rem;color:var(--text-3);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?= htmlspecialchars($ab['line_name'] ?? '—') ?></div>
            </div>
            <!-- Badge kondisi -->
            <div style="text-align:center;padding-top:3px;">
              <span style="display:inline-block;padding:3px 10px;border-radius:12px;font-size:.63rem;font-weight:800;text-transform:uppercase;letter-spacing:.5px;background:<?= $lBg ?>;color:<?= $lColor ?>;border:1px solid <?= $lBorder ?>;<?= $blink ?>">
                <?= $level ?>
              </span>
            </div>
            <!-- Daftar masalah -->
            <div style="padding-left:12px;display:flex;flex-direction:column;gap:3px;">
              ...
              ?>
              <span style="font-size:.68rem;font-weight:600;color:<?= $ic ?>;display:flex;align-items:center;gap:5px;">
...
/* ── OEE Chart per mesin — auto-scroll left/right ───────────── */
(function(){
  ...
  var idx      = 0;
  var autoPlay = true;
  var INTERVAL = 4500;   // ms per mesin
  var timer    = null;
...
(function(){
  ...
  function updateAbnormal(abnormal){
    var track = document.getElementById('alertTrack');
    if(!track) return;
    // Re-render seluruh track (data kecil, aman)
    if(!abnormal.length){
      track.innerHTML = '<div class="no-alert"><i class="fas fa-shield-alt"></i><div class="t1">Semua Mesin Normal</div><div class="t2">Tidak ada kondisi abnormal terdeteksi</div></div>';
      return;
    }
    var html = '';
    abnormal.forEach(function(ab){
      ...
      var issues = [];
      if(vs==='critical'||vs==='warning'){
        if(vx!==null) issues.push({sev:vs, txt:'Vib X: '+vx.toFixed(2)+' mm/s'+(vx>7.1?' (kritis)':vx>2.8?' (warning)':'')});
        if(vy!==null) issues.push({sev:vs, txt:'Vib Y: '+vy.toFixed(2)+' mm/s'+(vy>7.1?' (kritis)':vy>2.8?' (warning)':'')});
        if(vz!==null) issues.push({sev:vs, txt:'Vib Z: '+vz.toFixed(2)+' mm/s'+(vz>7.1?' (kritis)':vz>2.8?' (warning)':'')});
        if(rms>0)     issues.push({sev:vs, txt:'RMS Overall: '+rms.toFixed(2)+' mm/s'});
      }
      if(ab.status==='stop') issues.push({sev:'stop', txt:'Mesin tidak beroperasi'});
      if(tmp>50) issues.push({sev:'critical', txt:'Suhu Panel: '+tmp.toFixed(1)+' °C'});
      else if(tmp>40) issues.push({sev:'warn', txt:'Suhu Panel: '+tmp.toFixed(1)+' °C'});
  ...
  /* ── Update ticker (hanya rebuild kalau data berubah) ── */
  var _lastTickerHash = '';
  function updateTicker(machines){
    ...
    txt += '  ■ Live update ■ ';
    // Hanya update DOM jika konten berubah (mencegah animasi restart)
  ...
  /* ── Main fetch dengan isFetching guard ── */
  ...
  // Warm-up: jalankan setelah 2 detik pertama
