...
$pageTitle = 'Shift Production Input';
...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">
        <i class="fas fa-clipboard-list text-primary mr-2"></i>Shift Production Input
        <small class="text-muted ml-2" style="font-size:.75rem;">— Realtime OEE from Tower Lamp + Shift Data</small>
    </h1>
...
<div class="card shadow mb-4">
    <div class="card-body py-3">
        <div class="row align-items-center">
            <div class="col-md-4">
                <label class="font-weight-bold mb-1">Select Machine</label>
            ...
            <div class="col-md-3 mt-3 mt-md-0">
                <label class="font-weight-bold mb-1">Shift Production Target</label>
            ...
            <div class="col-md-3 mt-3 mt-md-0" id="shiftInfoBox" style="display:none;">
                <label class="font-weight-bold mb-1">Active Shift</label>
                <div>
                ...
                <div class="card-body">
                    <div class="row no-gutters align-items-center">
                        <div class="col mr-2">
                            ...
                            <small class="text-muted">Machine actively operating</small>
                        </div>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex align-items-center justify-content-between">
            <h6 class="m-0 font-weight-bold text-primary">
                <i class="fas fa-traffic-light mr-2"></i>Tower Lamp Status — Realtime
            </h6>
        ...
        <div class="card-body">
            <div class="row align-items-center">
                ...
                <!-- Durasi bar -->
                <div class="col-md-5">
                    <label class="font-weight-bold small">Shift Time Distribution</label>
                    <div id="timeDistBar" class="d-flex" style="height:20px;border-radius:4px;overflow:hidden;">
                        ...
                        <div id="tdStandby" style="background:#ffc107;height:100%;transition:width .5s;"
                            title="Standby"></div>
                        <div id="tdStop" style="background:#dc3545;height:100%;transition:width .5s;" title="Stop">
                        </div>
                        <div style="background:#e0e0e0;flex:1;" title="Remaining"></div>
                    </div>
                ...
                <div class="col-md-3 text-right">
                    <small class="text-muted d-block">Last state</small>
                    <strong id="lastStateTs" class="text-primary">—</strong>
                    <small class="text-muted d-block mt-1">Data from DS3231 RTC</small>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card shadow mb-4" id="closeShiftCard">
        <div class="card-header py-3 d-flex align-items-center justify-content-between">
            <h6 class="m-0 font-weight-bold text-danger">
                <i class="fas fa-flag-checkered mr-2"></i>Close Shift & Enter Rejects
            </h6>
            <span id="shiftStatusBadge" class="badge badge-success px-2 py-1">Shift Active</span>
        </div>
        <div class="card-body">
            <div id="shiftClosedMsg" style="display:none;">
                <div class="alert alert-info mb-0">
                    <i class="fas fa-info-circle mr-2"></i>Shift has been closed. OEE has been saved.
                </div>
            </div>
            <div id="shiftOpenForm">
                <div class="row">
                    <div class="col-md-3 mb-3">
                        <label class="font-weight-bold">Total Output</label>
                        <div class="input-group">
                            <input type="number" id="totalOutInput" class="form-control form-control-lg" placeholder="0"
                                min="0" value="0">
                            <div class="input-group-append">
                                <button class="btn btn-outline-secondary" onclick="manualIncrement()">+1</button>
                            </div>
                        </div>
                        <small class="text-muted">Total goods out this shift</small>
                    </div>
                    <div class="col-md-3 mb-3">
                        <label class="font-weight-bold text-danger">Total Rejects</label>
                        <input type="number" id="rejectInput" class="form-control form-control-lg" placeholder="0"
                            min="0" value="0">
                        <small class="text-muted">Filled in by operator at end of shift</small>
                    </div>
                    <div class="col-md-3 mb-3">
                        <label class="font-weight-bold">Operator Name</label>
                        <input type="text" id="operatorName" class="form-control" placeholder="Operator name">
                    </div>
                    <div class="col-md-3 mb-3">
                        <label class="font-weight-bold">Notes</label>
                        <input type="text" id="closeNotes" class="form-control" placeholder="Optional...">
                    </div>
                </div>
                <div class="row align-items-center mt-2">
                    <div class="col-md-6">
                        ...
                        <div class="alert alert-light border mb-0 py-2" id="previewOee" style="display:none;">
                            <small class="font-weight-bold">OEE Preview if closed now:</small>
                            <div class="mt-1">
                    ...
                    <div class="col-md-6 text-right">
                        ...
                        <button class="btn btn-danger" onclick="confirmCloseShift()" id="btnCloseShift">
                            <i class="fas fa-flag-checkered mr-1"></i>Close Shift & Save OEE
                        </button>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="row text-center">
                        <div class="col-md-2 col-6 mb-3">
                            <div class="small text-muted">SHT20 Temp</div>
                        ...
                        <div class="col-md-2 col-6 mb-3">
                            <div class="small text-muted">PZEM Voltage</div>
...
<style>
    .lamp-on-red {
        ...
        box-shadow: 0 0 18px rgba(220, 53, 69, .8) !important;
    }

    .lamp-on-yellow {
        background: #ffc107 !important;
        box-shadow: 0 0 18px rgba(255, 193, 7, .8) !important;
    }

    .lamp-on-green {
        background: #28a745 !important;
        box-shadow: 0 0 18px rgba(40, 167, 69, .8) !important;
    }
    ...
    .lamp-blink {
        animation: blinkLamp .6s infinite;
    }
</style>
...
<script>
    ...
    function updateShiftInfo(shift) {
        var names = ['', 'Morning Shift', 'Day Shift', 'Night Shift'];
        ...
        document.getElementById('shiftStatusBadge').textContent = closed ? 'Shift Closed' : 'Shift Active';
    ...
    function savePlan() {
        ...
        fetch('api/shift_production.php', {
            ...
            if (d.success) { showToast('Target saved: ' + qty + ' pcs', 'success'); loadShift(); }
        });
    }
    ...
    function confirmCloseShift() {
        ...
        if (reject > totalOut) { alert('Rejects cannot exceed total production.'); return; }
        if (!confirm('Are you sure you want to close the shift?\nTotal: ' + totalOut + ' | Rejects: ' + reject + '\nThis shift cannot be reopened.')) return;
        ...
        if (btnClose) { btnClose.disabled = true; btnClose.innerHTML = '<i class="fas fa-spinner fa-spin mr-1"></i>Saving...'; }
        ...
        fetch('api/shift_production.php', {
            ...
            .then(function (d) {
                if (btnClose) { btnClose.disabled = false; btnClose.innerHTML = '<i class="fas fa-flag-checkered mr-1"></i>Close Shift & Save OEE'; }
                if (d.success) {
                    showToast('Shift closed! OEE: ' + d.oee + '%  (A:' + d.availability + ' P:' + d.performance + ' Q:' + d.quality + ')', 'success');
                    loadShift();
                } else {
