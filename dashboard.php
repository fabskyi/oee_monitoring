...
if ($res) {
    while ($row = $res->fetch_assoc()) {
        $date = $row['snap_date'];
        $mid = $row['machine_id'];
        if (!in_array($date, $trendLabels))
            $trendLabels[] = $date;
        if (!isset($machineNames[$mid]))
            $machineNames[$mid] = $row['machine_name'];
...
if ($firstMid) {
    ...
    foreach ($trendLabels as $lbl)
        $points[] = isset($trendData[$firstMid][$lbl]) ? $trendData[$firstMid][$lbl] : 'null';
...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">
        ...
        <small class="text-muted ml-2" style="font-size:0.75rem;">
            &mdash; next update in <span id="countdown-val">10</span>s
        </small>
    </h1>
    <div class="d-flex align-items-center">
        <!-- MQTT realtime indicator -->
        <span id="mqttDot" title="Status MQTT" style="display:inline-block;width:9px;height:9px;border-radius:50%;
        ...
        <span id="syncDot" title="Connection status" style="display:inline-block;width:10px;height:10px;border-radius:50%;
...
<style>
    ...
    .mqtt-live-dot {
        animation: mqttPulse 1.5s infinite;
    }
    ...
    .status-changed {
        animation: statusChange 2s ease;
    }
                ...
                <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                        ...
                        <div class="h5 mb-0 font-weight-bold text-gray-800"><span
                                id="kpi-oee"><?php echo $kpiOee; ?></span>%</div>
                        <div class="progress mt-2" style="height:6px;">
                            <div id="kpi-oee-bar"
                                class="progress-bar <?php echo $kpiOee >= 85 ? 'bg-success' : ($kpiOee >= 60 ? 'bg-warning' : 'bg-danger'); ?>"
                ...
                <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                        ...
                        <div class="h5 mb-0 font-weight-bold text-gray-800">
                            <span id="kpi-running"><?php echo $runningMachines; ?></span> / <span
                                id="kpi-total"><?php echo $totalMachines; ?></span>
                        </div>
                        <small class="text-muted"><span
                                id="kpi-stopped"><?php echo $totalMachines - $runningMachines; ?></span> stopped</small>
                    </div>
            ...
            <div class="card-body">
                <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                        ...
                        <div class="h5 mb-0 font-weight-bold text-gray-800"><span
                                id="kpi-alerts"><?php echo $totalAlerts; ?></span></div>
                        <small class="text-muted" id="kpi-alerts-detail">
                            <?php if ($alertHigh): ?><span class="badge badge-danger">High:
                                    <?php echo $alertHigh; ?></span> <?php endif; ?>
                            <?php if ($alertMedium): ?><span class="badge badge-warning">Med:
                                    <?php echo $alertMedium; ?></span> <?php endif; ?>
                            <?php if ($alertLow): ?><span class="badge badge-secondary">Low:
                                    <?php echo $alertLow; ?></span><?php endif; ?>
    ...
    <div class="col-xl-3 col-md-6 mb-4">
        <div class="card border-left-info shadow h-100 py-2">
            <div class="card-body">
                <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                        <div class="text-xs font-weight-bold text-info text-uppercase mb-1">Maintenance Due (7 days)
                        </div>
                        <div class="h5 mb-0 font-weight-bold text-gray-800"><span
                                id="kpi-maint"><?php echo $maintenanceDue; ?></span></div>
            ...
            <div class="card-body d-flex flex-column align-items-center justify-content-center">
                ...
                <div class="mt-3 text-center small" id="donut-legend">
                    <span class="mr-2"><i class="fas fa-circle text-danger"></i> &lt;60%: <span
                            id="dist-low"><?php echo $distLow; ?></span></span>
                    <span class="mr-2"><i class="fas fa-circle text-warning"></i> 60-85%: <span
                            id="dist-mid"><?php echo $distMid; ?></span></span>
                    <span><i class="fas fa-circle text-success"></i> &gt;85%: <span
                            id="dist-high"><?php echo $distHigh; ?></span></span>
                </div>
            </div>
        </div>
    </div>
                        ...
                        foreach ($recentAlerts as $alert):
                            ...
                            if (!empty($alert['created_at'])) {
                                ...
                                if ($diff < 60)
                                    $ago = $diff . 's ago';
                                elseif ($diff < 3600)
                                    $ago = round($diff / 60) . 'm ago';
                                elseif ($diff < 86400)
                                    $ago = round($diff / 3600) . 'h ago';
                                else
                                    $ago = round($diff / 86400) . 'd ago';
                            }
                            ?>
                            <li class="list-group-item d-flex justify-content-between align-items-start py-2">
                                <div>
                                    <span
                                        class="badge badge-<?php echo $sevColor; ?> mr-1"><?php echo strtoupper($sev); ?></span>
                                    ...
                                    <small class="text-muted">
                                        ...
                                        &nbsp;(lo: <?php echo $alert['threshold_lo']; ?>, hi:
                                        <?php echo $alert['threshold_hi']; ?>)
                                    </small>
                                </div>
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-start py-2">
                                <div>
                                    ...
                                    <small class="text-muted">
                                        ...
                                        <?php if (!empty($mr['technician'])): ?>&mdash;
                                            <?php echo htmlspecialchars($mr['technician']); ?>        <?php endif; ?>
                                    </small>
                                </div>
...
<script>
    $(document).ready(function () {
        ...
        var countdownTimer;
        var tableCounter = 0;
        var tableEvery = Math.round(TBL_INTERVAL / KPI_INTERVAL);
        ...
        machineTable = $('#machineTable').DataTable({
            ...
            info: true,
            paging: true,
            language: { url: 'vendor/datatables/i18n/en.json' },
        ...
        function buildDataset(mid, colorIdx) {
            ...
            return {
                label: machineNames[mid], data: pts, borderColor: color,
        ...
        var threshDataset = {
            label: 'Target (85%)', data: lineLabels.map(function () { return 85; }),
            borderColor: '#858796', backgroundColor: 'transparent',
            borderDash: [6, 4], fill: false, tension: 0, pointRadius: 0, spanGaps: true
        };
        ...
        var lineChart = new Chart(lineCtx, {
            ...
            options: {
                ...
                scales: {
                    xAxes: [{
                        gridLines: { color: 'rgba(0,0,0,.05)' }, ticks: {
                            fontSize: 11,
                            callback: function (val) { if (!val) return val; var p = val.split('-'); return p[2] + '/' + p[1]; }
                        }
                    }],
                    yAxes: [{
                        ticks: { min: 0, max: 100, fontSize: 11, callback: function (v) { return v + '%'; } },
        ...
        function updateLiveStatus(data) {
            ...
            if (data.mqtt_live) {
                var ago = data.mqtt_secs < 60
                    ? data.mqtt_secs + 's ago'
                    : Math.round(data.mqtt_secs / 60) + 'm ago';
                setMqttDot('live', 'MQTT Live · ' + (data.mqtt_last || ''));
            } else if (data.mqtt_secs < 9999) {
                setMqttDot('nodata', 'MQTT ' + Math.round(data.mqtt_secs / 60) + 'm ago');
            } else {
